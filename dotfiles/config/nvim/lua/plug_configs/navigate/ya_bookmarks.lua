...
M.config = function()
  -- WARNING: исключать нефайловые и term:// буферы
  local calibrate = require("bookmarks.calibrate")
  local orig = calibrate.calibrate_current_window
  calibrate.calibrate_current_window = function()
    ...
    if path == "" or path:sub(1, 7) == "term://" then
      return
    end
    return orig()
  end
